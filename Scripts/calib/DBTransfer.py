...
class DBTransferManager:
	...
	def transfer_lines(self, tbl, qry, xargs = {}, fields = None, subgraphs = [], withPrimary = False):
		...
		for z in datlist:
			...
			cmd = "INSERT INTO `" + tbl + "` (" + join(["`%s`"%str(i) for i in fdict.keys()],',') + ") VALUES (" + join(["'%s'"%str(i) for i in fdict.values()],',') + ")"
			print cmd
			if self.connOut is not None:
				self.connOut.execute(cmd)
				self.connOut.execute("SELECT LAST_INSERT_ID()")
				inserts.append( int(self.connOut.fetchone()[0]) )
		
		if self.connOut is None:
			return [-1,]
		return inserts
	...
	def transfer_graph(self,gid):
		...
		iid = self.transfer_lines("graphs","graph_id = %i"%gid)[0]
	...
	def transfer_energy_calibration(self,ecid,pmid):
		iid = self.transfer_lines("energy_calibration", "ecal_id = %i"%ecid, {"posmap_set_id":pmid})[0]
	...
	def transfer_mwpc_ecal(self,ecid,pmid):
		return self.transfer_lines("mwpc_ecal", "mwpc_ecal_id = %i"%ecid, {"gain_posmap_id":pmid})[0]

	def transfer_evis_conversion(self,evcid):
		return self.transfer_lines("evis_conversion", "evis_conversion_id = %i"%evcid, subgraphs=["conversion_curve_id"])[0]

	def transfer_cathscale(self,csid):
		return self.transfer_lines("cath_ccloud_scale","cath_ccloud_scale_id = %i"%csid, subgraphs=["gain_graph_id"])[0]

	def transfer_cathcal_set(self, csid, transferShape=False):
		ncsid = self.transfer_lines("cathcal_set", "cathcal_set_id = %i"%csid)[0]
		...
		for ocsgid in ocsgids:
			ncsgid = self.transfer_lines("cathseg_cal", "cathseg_id = %i"%ocsgid, {"cathcal_set_id":ncsid})[0]
			if transferShape:

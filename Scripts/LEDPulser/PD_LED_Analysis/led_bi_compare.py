...
#            print LED_PD_array[i]['Mu']
#            print LED_PMT_array[i]['Mu']
...
def average_diff_over_west_tubes():
    # average over (Bi-LED/PD) for tubes 4,5,6 
    # (tube 7 Bi pulser doesn't work)
    # to create a run-by-run correction function 
    
    # fill bi - led/pd arrays
    list_of_datasets = list()
    list_of_runlists = list()
    for i in range(4,7):  # selects tubes 4,5,6. Thanks, wierd python convention!
        arrays_temp = make_led_bi_diff_array(i) # returns [[data],[errs],[runs]] 
        data = arrays_temp[0]
        runs = arrays_temp[1]
        list_of_datasets.append(data)
        list_of_runlists.append(runs)

    # sync up run lists
    common_run_list = list_of_runlists[0]
    for runlist in list_of_runlists:
        print "making common run list" 
        common_run_list = find_array_overlap(common_run_list, runlist)

    # make new data arrays only containing common runs
    list_of_datasets_common = list()
    for i in arange(len(list_of_runlists)):  # 3 runlists
        data_common = list()
        tube_runlist = list_of_runlists[i]
        # scan over runs in common runlist and append entries for only those runs
        for common_run in common_run_list: # N common runs
            for k in arange(len(tube_runlist)):  # N + (# unique runs) 
                if float(tube_runlist[k]) == float(common_run):
                    data_entry = list_of_datasets[i][k]
                    data_common.append(data_entry)
        list_of_datasets_common.append(data_common) #store new datasets
        
    average_dataset = list()
    for m in arange(len(common_run_list)): # N runs need N datapoints
        average_point = 0
        for n in arange(len(list_of_datasets_common)): # 3 datasets
            average_point += list_of_datasets_common[n][m] # grab point m from nth dataset
        average_point = average_point/3.   # normalize average
        average_dataset.append(average_point)
        
    output = [average_dataset, common_run_list]

    return output

# mainly for testing
def make_west_average_plot():
    fig, ax = plt.subplots()
    
    dataset_runlist = average_diff_over_west_tubes() 
    dataset = dataset_runlist[0]
    runlist = dataset_runlist[1]

    ax.errorbar(runlist, dataset, 
...
def find_good_runs(dataset):
    # known bad runs culled by hand
    bad_runlist = [20782, 21146, 21167, 21201, 21295, 21367, 21368, 
                   21372, 21374, 21480, 21589, 21834, 22157, 22374,
                   22375, 22377, 22383]
    ...
    for row in dataset:
        #skip known bad runs
        bad_bool = 0
        for m in bad_runlist: 
            if float(row['Run']) == float(m):
                bad_bool = 1
        if bad_bool == 1:
            continue
        # if desired value is > 0, append run to good runlist 
        if float(row['Mu']) > 0.0:
...
def make_bi_ax(ax, tubeIn, normBool=1):
    ...
    #returns [data_array, errs_array, runs_array]
    bi_arrays = make_bi_arrays(tubeIn, normBool)
    bi_plot_data = bi_arrays[0]
    bi_plot_errs = bi_arrays[1]
...
def make_bi_arrays(tubeIn, normBool=1):
    bi_runlist = GetBiRuns()
    ...
    #redefine arrays to remove entries where bi_plot_data is 0 
    stripped_data = strip_zeroes(bi_plot_data, bi_runlist)
    stripped_errs = strip_zeroes(bi_plot_data, bi_plot_errs)
    bi_plot_data = stripped_data[0]
    bi_runlist = stripped_data[1]
    bi_plot_errs = stripped_errs[1]
    
    output = [bi_plot_data, bi_plot_errs, bi_runlist]
    return output

def make_bi_diff_arrays(tube1, tube2, normBool=1):
    markSize = 4
    
    bi_arrays_1 = make_bi_arrays(tube1, normBool)
    bi_plot_data_1 = bi_arrays_1[0]
    bi_plot_errs_1 = bi_arrays_1[1]
    bi_runlist_1 = bi_arrays_1[2]

    bi_arrays_2 = make_bi_arrays(tube2, normBool)
    bi_plot_data_2 = bi_arrays_2[0]
    bi_plot_errs_2 = bi_arrays_2[1]
    bi_runlist_2 = bi_arrays_2[2]
    
    runlist_out = list()
    biArrayDiff = list()
    biArrayErrs = list()
    for i in arange(len(bi_runlist_1)):
        for j in arange(len(bi_runlist_2)):
            if bi_runlist_1[i] == bi_runlist_2[j]:
#                print bi_runlist_1[i]
                runlist_out.append(bi_runlist_1[i])
                diff = bi_plot_data_1[i] - bi_plot_data_2[j]
                biArrayDiff.append(diff)
                err = (1./1.414)*(bi_plot_errs_1[i]*bi_plot_errs_1[i] + 
                                  bi_plot_errs_2[j]*bi_plot_errs_2[j])
                biArrayErrs.append(err)
                
    output = [biArrayDiff, biArrayErrs, runlist_out]
    return output

def make_bi_diff_ax(ax, tube1, tube2, normBool =1):
#def make_bi_diff_ax(tube1, tube2, normBool =1):
    markSize = 4

    #returns [data_array, errs_array, runs_array]
    bi_arrays = make_bi_diff_arrays(tube1, tube2, normBool)
    bi_plot_data = bi_arrays[0]
    bi_plot_errs = bi_arrays[1]
    bi_runlist = bi_arrays[2]
 ...
 #       ax.set_ylim(Biavg - 0.2, Biavg + 0.2)
    ...
    #    ax.set_ylim(Biavg-500, Biavg+500)
    #    ax.set_ylabel("PMT ADC")
    title = "Bi Pulser Tube " + str(tube1) + " - Tube " + str(tube2)
    ax.set_title(title)
...
def make_all_tube_bi_diffs_for_tube(tubeIn):
    for i in range(8):
#        print i
        fig, ax = plt.subplots()
        ax = make_bi_diff_ax(ax, tubeIn, i)
        ax.set_ylim(-0.5, 0.5)
...
def make_led_arrays(tubeIn, normBool=1):
    ...
    #redefine arrays to remove entries where bi_plot_data is 0 
    stripped_data = strip_zeroes(led_pmt_plot_data, led_pmt_runlist)
    stripped_errs = strip_zeroes(led_pmt_plot_data, led_pmt_plot_errs)
    led_pmt_plot_data = stripped_data[0]
    led_pmt_runlist = stripped_data[1]
    led_pmt_plot_errs = stripped_errs[1]
    
    output = [led_pmt_plot_data, led_pmt_plot_errs, led_pmt_runlist]
    return output

def make_led_diff_arrays(tube1, tube2, normBool=1):
    led_pmt_arrays_1 = make_led_arrays(tube1, normBool)
    led_pmt_plot_data_1 = led_pmt_arrays_1[0]
    led_pmt_plot_errs_1 = led_pmt_arrays_1[1]
    led_pmt_runlist_1 = led_pmt_arrays_1[2]

    led_pmt_arrays_2 = make_led_arrays(tube2, normBool)
    led_pmt_plot_data_2 = led_pmt_arrays_2[0]
    led_pmt_plot_errs_2 = led_pmt_arrays_2[1]
    led_pmt_runlist_2 = led_pmt_arrays_2[2]
    
    runlist_out = list()
    ledArrayDiff = list()
    ledArrayErrs = list()
    for i in arange(len(led_pmt_runlist_1)):
        for j in arange(len(led_pmt_runlist_2)):
            if led_pmt_runlist_1[i] == led_pmt_runlist_2[j]:
#                print bi_runlist_1[i]
                runlist_out.append(led_pmt_runlist_1[i])
                diff = led_pmt_plot_data_1[i] - led_pmt_plot_data_2[j]
                ledArrayDiff.append(diff)
                err = (1./1.414)*(led_pmt_plot_errs_1[i]*
                                  led_pmt_plot_errs_1[i] + 
                                  led_pmt_plot_errs_2[j]*
                                  led_pmt_plot_errs_2[j])
                ledArrayErrs.append(err)
                
    output = [ledArrayDiff, ledArrayErrs, runlist_out]
    return output

def make_led_diff_ax(ax, tube1, tube2, normBool =1):
#def make_led_diff_ax(tube1, tube2, normBool =1):
    markSize = 4

    #returns [data_array, errs_array, runs_array]
    led_pmt_arrays = make_led_diff_arrays(tube1, tube2, normBool)
    led_pmt_plot_data = led_pmt_arrays[0]
    led_pmt_plot_errs = led_pmt_arrays[1]
    led_pmt_runlist = led_pmt_arrays[2]

    title = "LED Pulser: Tube " + str(tube1) + " - Tube " + str(tube2)

#    fig, ax = plt.subplots()
    ax.set_title(title)

    ax.errorbar(led_pmt_runlist, led_pmt_plot_data, yerr = led_pmt_plot_errs, 
                   linestyle='None', marker='o', markersize=markSize)

    return ax
#    return fig

def make_all_led_diff_plots_for_tube(tubeIn):
    for i in range(8):
#        print i
        fig, ax = plt.subplots()
        ax = make_led_diff_ax(ax, tubeIn, i)
        ax.set_ylim(-0.5, 0.5)
...
def make_led_bi_diff_array(tubeIn, div=0):
    led_pmt_data = getLEDDataforTube(tubeIn)
    led_pmt_data = process_LED_PD_corr(led_pmt_data)
    led_pmt_plot_data = led_pmt_data['Mu'] 
    led_pmt_plot_errs = led_pmt_data['MuErr']
    ...
    # These runs have missing or poorly identified 
    # data in PD gain region but still show up on plot
    bad_runlist = [20782, 21146, 21167, 21201, 21295, 21367, 21368, 
                   21372, 21374, 21480, 21589, 21834, 22157, 22374,
                   22375, 22377, 22383]
    ...
    for j in arange(len(bi_runlist)):
        ...
        for k in arange(len(led_pmt_data)):
            if float(led_pmt_data[k]['Run']) == float(bi_runlist[j]): # compare run No.
                ...
                bad_bool = 0
                for m in bad_runlist: 
                    if float(led_pmt_data[k]['Run']) == float(m):
                        bad_bool = 1
                if bad_bool == 1:
                    continue
                ...
                # reject points where either value is 0
                if led_pmt_plot_data[k] == 0.0:
                    continue 
    ...
    #redefine arrays without any 0s 
    zipped_stripped = strip_zeroes(diff_list, bi_runlist)
         
#    return diff_list
    return zipped_stripped

def make_led_bi_diff_plot(tubeIn, div = 0):
    markSize = 4

    # make_led_bi_diff_array contains zero stripping, 
    # providing a fixed runlist to plot with

    #    diff_array = make_led_bi_diff_array(tubeIn, div)
    zipped_stripped = make_led_bi_diff_array(tubeIn, div)
    diff_array = zipped_stripped[0]
    bi_runlist = zipped_stripped[1]
#    bi_runlist = GetBiRuns()    
    ...
    ax_led_pmt_diff.set_title("Bi Minus LED/PD, Tube " + str(tubeIn))
...
#    diffArray1 = make_led_bi_diff_array(tube1, 0)
#    diffArray2 = make_led_bi_diff_array(tube2, 0)
# bi_runlist = GetBiRuns()
    zipped_stripped1 = make_led_bi_diff_array(tube1, 0)
    diffArray1 = zipped_stripped1[0]
    runlist1 = zipped_stripped1[1]
    zipped_stripped2 = make_led_bi_diff_array(tube2, 0)  
    diffArray2 = zipped_stripped2[0]
    runlist2 = zipped_stripped2[1]

    # sync both run lists to use same runs
    runlistBoth = list()
#    diffArray1_sync = list()
#    diffArray2_sync = list()
    diffArrayCompare = list()

    for i in arange(len(runlist1)):
        for j in arange(len(runlist2)):
            run1 = runlist1[i]
            run2 = runlist2[j]
            if run1 == run2:
                runlistBoth.append(run1)
                diff = diffArray1[i] - diffArray2[j]
                diffArrayCompare.append(diff)
 ...
 #   axis.errorbar(bi_runlist, diffArrayCompare, 
    axis.errorbar(runlistBoth, diffArrayCompare, 
                  linestyle='None', marker='o', markersize=markSize)
...
#    for i in arange(len(axlist)): 
    ...
    #        axlist[i].set_ylim(-0.2, 0.2)
        ax.set_ylim(-0.2, 0.2)
        title = "Tube " + str(tubeIn) + " - Tube " + str(i)
        ax.set_title(title)
        ax.set_xlabel("Run Number")
        ax.set_ylabel("normalized difference")
...
def normalize_array_to_avg(array):
    total = sum(array)
    average = total/float(len(array))
    retarray = normalize_array(array, average)
    return retarray
...
def make_plot(tubeIn, normBool=1, corrBool=1, fitBool=0):
    ...
    if corrBool == 1:
        ax_led_pmt.set_title("(LED Pulser)/PD, Tube " + str(tubeIn))
...
def strip_zeroes(in_array_vals, in_array_runs):
    # iterate over entries of in_array_vals, 
    # and only keep the entries != 0 
    if len(in_array_vals) != len(in_array_runs):
        print "Error in strip_zeroes: Arrays not the same length."
        return -1

    print "Stripping zeroes from array of length " +str(len(in_array_vals))

    out_array_vals = list()
    out_array_runs = list()
    for i in arange(len(in_array_vals)):
        if in_array_vals[i] != 0.0:
            out_array_vals.append(in_array_vals[i])
            out_array_runs.append(in_array_runs[i])

    out = zip(out_array_vals, out_array_runs)
    out = zip(*out) #now out = [out_array_vals, out_array_runs]
    print "Returning array of length " + str(len(out_array_vals))

    return out

def make_all_plots(save, corr, filename):
    if save:
        #outputfile = PdfPages("/data4/saslutsky/PulserComp/imagesLEDDebug/BiLEDcompare_PDcorrected_longrange_culled.pdf")
#        path = "/data4/saslutsky/PulserComp/imagesLEDDebug/"
        path = "/data4/saslutsky/PulserComp/imagesL_06_09_2014/"
...
def make_all_diff_plots(div = 0, save = 0):
    if save:
        outputfile = PdfPages("/data4/saslutsky/PulserComp/imagesLEDDebug/BiLEDcompare_diff_culled.pdf")
    for tube in range(8):
        print "Making Diff Plot " + str(tube)
        figr = make_led_bi_diff_plot(tube, div)
        if save:
            outputfile.savefig(figr)
    if save: 
        outputfile.close()

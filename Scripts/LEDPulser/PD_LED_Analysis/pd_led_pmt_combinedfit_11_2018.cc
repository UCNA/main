...
int main (int argc, char **argv)
{
    ...
    // COMSOL: /data1/saslutsky/ucnadata/2011/rootfiles
    TChain h1("h1");
...
//            100, 200, 300);
    ...
    TString rawfilename = "RawGraphData";
    rawfilename = OUTPUT_IMAGE_DIR + rawfilename + "_";
    rawfilename += argv[1];
    rawfilename += ".txt";
    ofstream rawfile;
...
// Both PMT and PD generate two curves, one for each LED. Good cross-check, I guess? 
// Will end up with just two curves for each PMT, as before. Although they should be essentially the same this time. 
...
//   for (int led = 0; led < 2; led++){
//       for (int i = 0; i < NUM_CHANNELS; i++){
//   	    for(int s = 0; s < gPMT[led][i].size(); s++){ 
//	       _gPMTval = gPMT[led][i][s]; 
//	       _p0 = fittedFunctions[led][i].GetParameter(0);
//	       _p1 = fittedFunctions[led][i].GetParameter(1);
//	       _p2 = fittedFunctions[led][i].GetParameter(2);
//	       _eta_LED = fittedFunctions[led][i].GetParameter(5);
//               gPMT_light[led][i].push_back( (_p0 + _p1*_gPMTval + _p2*_gPMTval*_gPMTval) / _eta_LED );  
//               std::cout << led << " " << i << " " << gPD_light[led][i][s] << " " << gPMT_light[led][i][s] << std::endl;
//            }
//	}
//   }
...
//                if (ew) light_canvas_W->cd(tx+2*ty+1);
		...
		gStyle->SetOptFit(0);
                ...
                PMT_light_graph[DOWN][i]->SetName("LightGraphDOWN");
                PMT_light_graph[UP][i]->SetTitle(title);

...
def plot_ped_history():
	...
	timeaxis = graph.axis.lin(title="Beta Runs, PMT Peds (2011/2012)",parter=myparter,texter=mytexter,min=pdat[0][0].midTime()-86400,max=pdat[-1][0].midTime()+86400)
	c = canvas.canvas()
	c1 = canvas.canvas()
	glist = []
	alist = []
	...
	for s in ['East','West']:
		...
		for t in range(4):
			...
			avmon=graph.graphxy(width=25,height=gheight,ypos=goff,
                                x=aaxis,
                                y=graph.axis.lin(title="%s %i"%(s,t+1),min=mu-20,max=mu+20,parter=graph.axis.parter.linear(tickdists=[20,5])),
                                key = None)
			#setTexrunner(grmon)
			c.insert(grmon)
			c1.insert(avmon)
...
def plot_gms_history():
	...
	for s in ['East','West']:
		...
		for t in range(4):
			print s,t
			k = (s[0],t)
			gdat = [(p[0].midTime(), 100*(p[2][k]/p[1][k]-1), 100*p[3][k]/p[1][k], p[0]) for p in pdat if p[4][k]>4]
			#gdat = [(p[0].midTime(), p[3][k], p[4][k], p[0]) for p in pdat if p[4][k]>4]
			mu,sigma = musigma([g[1] for g in gdat])
			print gdat[0][0], gdat[-1][0], mu, sigma
			yrange = 17
	...
	c.writetofile("/media/Data2/Analysis_Output/test/CalPlots/PMTBiGMS_Beta_20112012.pdf")
...
if __name__ == "__main__":
	...
	#plot_trigeff_history(16606,19966)

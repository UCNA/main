...
for run in octetRuns:
    cmd = "./MC_Plugin_Analyzer beta %s %s" %(outputDir,run)
    #os.system("cd ../../; %s"%(cmd))
    print cmd
    os.system("cd %s/hists/; mv %s_sim.root Octet_%i_%s-%s/"%(outputDir,run,octetNum,octetRuns[0],octetRuns[len(octetRuns)-1]))

...
class GeantSimManager:
	...
	def launch_sims(self, maxIn=100000, hours_old=0):
		...
		for rn in range(nruns):
			...
			self.settings["nevt"] = 100000000	# actual limit is number of events in file, nominally 10000
...
if __name__ == "__main__":
	...
	# MC_EventGen is an executable and can be generated by running 'make MC_EventGen'
        # in the base directory where the code lives.
	######################
	if options.calsrcs:
	...
	if options.xesrcs:
		...
		for g in XeIsots:
			sourceSim = GeantSimManager("CORRECT_2012-2013_isobutane", geometry="2012/2013_isobutane")
			sourceSim.set_evtsrc(g)
			if options.sim:
...
######################## Neutrons 2011-2013 ###############################
	if options.neutrons:
		...
		betaSim = GeantSimManager("XuanTest_SettingsON_saveAll", geometry="2011/2012",vacuum="1e-5 torr")#, fmap=os.environ["UCNA_AUX"]+"/UniformFieldmap.txt")
	#betaSim.set_generator("neutronBetaUnpol")
		...
		if options.sim:
			...
			betaSim.launch_sims(maxIn=30000)
...
if False:
	...
	if 0:
		betaSim = GeantSimManager("20120824_MagF",vacuum="1.e-3 torr",fmap=os.environ["UCNA_AUX"]+"/Fieldmap_20101028_b.txt")
		betaSim.set_generator("neutronBetaUnpol")
		betaSim.settings["extra_cmds"] += "/detector/rotation 0.037\n"
		betaSim.settings["extra_cmds"] += "/detector/offset -3.98 0.44 0 mm\n"
	...
	if 0:
		for g in ["Bi207","Sn113","Ce139"]:
			sourceSim = GeantSimManager("20120823",fmap=os.environ["UCNA_AUX"]+"/Fieldmap_20101028_b.txt",geometry="C")
			sourceSim.g4_out_dir_base = os.environ["G4OUTDIR"]
			sourceSim.set_evtsrc(g)
			sourceSim.launch_postanalyzer()
		exit(0)
	...
	if 0:
		for g in ["In114E","In114W"]:
			sourceSim = GeantSimManager("NEW_2011-2012",fmap=os.environ["UCNA_AUX"]+"/Fieldmap_20101028_b.txt",geometry="2011/2012")
	...
	if 0:
		for g in ["Bi207","Sn113","Ce139"]:
			sourceSim = GeantSimManager("DeepCrinkle",fmap=os.environ["UCNA_AUX"]+"/Fieldmap_20101028_b.txt")
			sourceSim.settings["physlist"]="livermore"
			sourceSim.settings["sourceScan"]=80.

...
struct ucnaDAQEvt {
	...
	Int_t subEvt;									///< sub-event enumerator for multiple triggers
	
	Float_t cathCharge[BOTH][Y_DIRECTION+1][kMaxCathodes];		///< estimated cathode segment charge signal
	
	/// clear all values for new event
	void reset(bool calcCathCharge);
	/// normalize position average variables by total weight
	void calcPositions();
};
...
public:
	...
	ucnaDAQEvt DE0;							///< DAQ variables for event
	std::vector<ucnaDAQEvt> subEvts;		///< events for each main trigger
	std::vector<double>	scintHitTimes;		///< trigger-generating scintillator hit times
	std::vector<double> triggerTimes;		///< start times for scintillator triggers
	
protected:

	Double_t circ_pts[Y_DIRECTION+1][N_CHG_CIRC_PTS];			///< positions of points on a circle, for wirechamber charge spread

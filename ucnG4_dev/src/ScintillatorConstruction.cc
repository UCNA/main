...
void ScintillatorConstruction::Construct(Side sd) {
	...
	// overall container
	G4Tubs* N2_vol_tube = new G4Tubs(sideSubst("N2_vol_tube%c",sd),0.,backing_Radius,N2_volume_Z/2,0.,2*M_PI);
	container_log = new G4LogicalVolume(N2_vol_tube,WCNitrogen,sideSubst("N2_vol_log%c",sd));
	container_log->SetVisAttributes(G4VisAttributes::Invisible);
	
	// dead layer
	G4Tubs* Dscint_tube = new G4Tubs(sideSubst("Dscint_tube%c",sd),0.,scint_Radius,dead_thick/2,0.,2*M_PI);
	Dscint_log = new G4LogicalVolume(Dscint_tube,Sci,sideSubst("Dscint_log%c",sd));
	...
	Dscint_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,-(N2_volume_Z-dead_thick)/2.),
									Dscint_log,sideSubst("Dscint_phys%c",sd),container_log,false,0);

	// scintillator
	G4Tubs* scint_tube = new G4Tubs("scint_tube",0.,scint_Radius,(scint_thick-dead_thick)/2.,0.,2*M_PI);
	scint_log = new G4LogicalVolume(scint_tube,Sci,sideSubst("scint_log%c",sd));
	...
	scint_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,-N2_volume_Z/2+dead_thick+(scint_thick-dead_thick)/2.),
								   scint_log,sideSubst("scint_phys%c",sd),container_log,false,0);
	...
	lightguide_log = new G4LogicalVolume(lightguide_polycone,Sci,sideSubst("lightguide_log%c",sd));
	...
	lightguide_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,-N2_volume_Z/2.),
										lightguide_log,sideSubst("scint_phys%c",sd),container_log,false,0);
	
	// backing veto
	G4Tubs* backing_tube = new G4Tubs("backing_tube",0.,backing_Radius,backing_thick/2.,0.,2*M_PI);
	backing_log = new G4LogicalVolume(backing_tube,Sci,sideSubst("backing_log%c",sd));
	...
	backing_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,(N2_volume_Z-backing_thick)/2.),
										backing_log,sideSubst("backing_phys%c",sd),container_log,false,0);

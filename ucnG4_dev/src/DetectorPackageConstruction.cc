...
void DetectorPackageConstruction::Construct(Side sd) {
	scint.Construct(sd);
	mwpc.Construct(sd);
	...
	mwpc_entrance_log = new G4LogicalVolume(mwpc_entrance_tube, Vacuum, sideSubst("mwpc_entrance_log%c",sd));
	mwpc_entrance_log->SetVisAttributes(G4VisAttributes::Invisible);
	entrance_front_log = new G4LogicalVolume(entrance_front_tube, Al, sideSubst("entrance_front_log%c",sd));
	entrance_mid_log = new G4LogicalVolume(entrance_mid_tube, Al, sideSubst("entrance_mid_log%c",sd));
	entrance_back_log = new G4LogicalVolume(entrance_back_tube, Al, sideSubst("entrance_back_log%c",sd));
	...
	entrance_front_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,-0.5*(entrance_section_length-mwpc_entrance_thickness)),
											entrance_front_log,sideSubst("entrance_front%c",sd),mwpc_entrance_log,false,0);
	entrance_mid_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,-0.5*frontwin_frame_thick),
										  entrance_mid_log,sideSubst("entrance_mid%c",sd),mwpc_entrance_log,false,0);
	entrance_back_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,0.5*(entrance_section_length-frontwin_frame_thick)),
										   entrance_back_log,sideSubst("entrance_back%c",sd),mwpc_entrance_log,false,0);
	...
	G4Tubs* detPackage_tube = new G4Tubs(sideSubst("detPackage_tube%c",sd),0,detPackageRadius,detPackageHalfZ,0.,2*M_PI);   
	container_log = new G4LogicalVolume(detPackage_tube,Vacuum,sideSubst("container_log%c",sd));
	...
	scint_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,-scint.getScintFacePos()),
								   scint.container_log,sideSubst("N2_vol_phys%c",sd),container_log,false,0);
	...
	mwpc_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,mwpc_pos),
								  mwpc.container_log,sideSubst("mwpcContainer%c",sd),container_log,false,0);
	...
	mwpc_entrance_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,entrance_pos),
										   mwpc_entrance_log,sideSubst("mwpc_entrance%c",sd),container_log,false,0);
	...
	mwpc_exit_log = new G4LogicalVolume(mwpc_exit_tube, Al, sideSubst("mwpc_exit_log%c",sd));
	...
	mwpc_exit_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,exit_frame_pos),
									   mwpc_exit_log,sideSubst("mwpc_exit%c",sd),container_log,false,0);	
	G4Tubs* mwpc_exit_N2_tube = new G4Tubs("mwpc_exit_N2_tube",0,mwpc.mwpc_exit_R,0.5*backwin_frame_thick,0.,2*M_PI);
	mwpc_exit_N2_log = new G4LogicalVolume(mwpc_exit_N2_tube, WCNitrogen, sideSubst("mwpc_exit_N2_log%c",sd));
	...
	mwpc_exit_N2_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,exit_frame_pos),
										  mwpc_exit_N2_log,sideSubst("mwpc_exit%c",sd),container_log,false,0);
	...
	G4Tubs* backstuff_tube = new G4Tubs(sideSubst("backstuff_tube%c",sd),0,detPackageRadius,backstuff_thick*0.5,0.,2*M_PI); 
	backstuff_log = new G4LogicalVolume(backstuff_tube,SS304,sideSubst("backstuff_log%c",sd));
	backstuff_phys = new G4PVPlacement(NULL,G4ThreeVector(0.,0.,detPackageHalfZ-0.5*backstuff_thick),
									   backstuff_log,sideSubst("backstuff%c",sd),container_log,false,0);
}

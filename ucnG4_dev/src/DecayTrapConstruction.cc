...
void DecayTrapConstruction::Construct(G4LogicalVolume* world, double crinkleAngle) 
{
	...
	for(Side sd = EAST; sd <= WEST; ++sd) {
	        ...
	        thicknessOfTrapWindow[sd] = fWindowThick[sd]+fCoatingThick[sd];		
                beWinPosZ[sd] = fCoatingThick[sd]/2.-thicknessOfTrapWindow[sd]/2.;
	        mylarWinPosZ[sd] = thicknessOfTrapWindow[sd]/2.-fWindowThick[sd]/2.;
	        trap_winPosZ[sd] = (decayTubeL[sd]+thicknessOfTrapWindow[sd])/2.;
	
                trap_win_tube[sd] = new G4Tubs("trap_win_tube",0.,decayTube_OR,thicknessOfTrapWindow[sd]/2.,0.,2*M_PI);
                mylarTube[sd] = new G4Tubs("mylarTube",0.,decayTube_OR,fWindowThick[sd]/2.,0.,2*M_PI);
                beTube[sd] = new G4Tubs("beTube",0.,decayTube_OR,fCoatingThick[sd]/2.,0.,2*M_PI);
		...
		wigglefoils[sd].addLayer(fWindowMat,fWindowThick[sd],true,visWindow);
		wigglefoils[sd].addLayer(fCoatingMat,fCoatingThick[sd],true,visWindow);
		wigglefoils[sd].Construct();
		
		trap_win_log[sd] = new G4LogicalVolume(trap_win_tube[sd],Vacuum,sideSubst("trap_win_log%c",sd));

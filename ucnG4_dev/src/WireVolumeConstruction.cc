...
void WireVolumeConstruction::Construct(Side sd) {
	...
	gas_log = new G4LogicalVolume(mwpc_box,fMWPCGas,sideSubst("mwpc_log%c",sd));
	...
	cathSeg_log = new G4LogicalVolume(cathodeSegmentBox,fMWPCGas,sideSubst("cathSeg_log%c",sd));
	cathSeg_log->SetVisAttributes(G4VisAttributes::Invisible);
	anodeSeg_log = new G4LogicalVolume(anodeSegmentBox,fMWPCGas,sideSubst("anodeSeg_log%c",sd));
	anodeSeg_log->SetVisAttributes(G4VisAttributes::Invisible);
	cathode_wire_log = new G4LogicalVolume(cathode_tube,fCathodeWireMaterial,sideSubst("cathode_log%c",sd));
	cathode_wire_log->SetVisAttributes(visCathWires);
	cath_plate_log = new G4LogicalVolume(cathplate_tube,fCathodePlateMaterial,sideSubst("cathplate_log%c",sd));
	cath_plate_log->SetVisAttributes(visCathWires);
	anode_wire_log = new G4LogicalVolume(anode_tube,fAnodeWireMaterial,sideSubst("anode_log%c",sd));				
	anode_wire_log->SetVisAttributes(visAnodeWires);
	
	new G4PVPlacement(NULL,G4ThreeVector(),cathode_wire_log,
					  sideSubst("cathode_wire_phys%c",sd),cathSeg_log,true,0);
	new G4PVPlacement(NULL,G4ThreeVector(),cath_plate_log,
					  sideSubst("cath_plate_phys%c",sd),cathSeg_log,true,0);
	new G4PVPlacement(NULL,G4ThreeVector(),anode_wire_log,
					  sideSubst("anode_wire_phys%c",sd),anodeSeg_log,true,0);
	
	
	// anode, cathode plane container volumes
	G4LogicalVolume* cathContainer1_log = new G4LogicalVolume(cathContainer_box,fMWPCGas,sideSubst("cathContainer1_log%c",sd));
	G4LogicalVolume* cathContainer2_log = new G4LogicalVolume(cathContainer_box,fMWPCGas,sideSubst("cathContainer2_log%c",sd));
	G4LogicalVolume* anodContainer_log = new G4LogicalVolume(anodeContainer_box,fMWPCGas,sideSubst("anodContainer_log%c",sd));
	
	new G4PVPlacement(xRot90,G4ThreeVector(0.,0.,cathode_R-planeSpacing),
					  cathContainer1_log,sideSubst("cathContainer1_phys%c",sd),gas_log,false,0);
	new G4PVPlacement(xzRot90,G4ThreeVector(0.,0.,planeSpacing-cathode_R),
					  cathContainer2_log,sideSubst("cathContainer2_phys%c",sd),gas_log,false,0);
	new G4PVPlacement(xRot90,G4ThreeVector(0.,0.,0.),
					  anodContainer_log,sideSubst("anodContainer_phys%c",sd),gas_log,false,0);
	
	// replicate segments into cathode, anode arrays
	new G4PVReplica(sideSubst("cathode_array_1_%c",sd),
					cathSeg_log,
					cathContainer1_log,
					kXAxis,
					NbOfWires,
	...
	new G4PVReplica(sideSubst("cathode_array_2_%c",sd),
					cathSeg_log,
					cathContainer2_log,
					kXAxis,
					NbOfWires,
	...
	new G4PVReplica(sideSubst("anode_array_%c",sd),
					anodeSeg_log,
					anodContainer_log,
					kXAxis,
					NbOfWires,

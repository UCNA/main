...
void bmPrimaryGeneratorAction::throwEvents(const std::vector<NucDecayEvent>& evts, G4Event* anEvent) {
	if(!evts.size()) return;
...
void bmPrimaryGeneratorAction::SetEventFile(G4String val) {
...
bmPrimaryGeneratorAction::bmPrimaryGeneratorAction(bmDetectorConstruction* myDC): myDetector(myDC), ETS(NULL), sourceRadius(0), relToSourceHolder(false) {
...
void bmPrimaryGeneratorAction::setVertices(std::vector<NucDecayEvent>& v) {
...
void bmPrimaryGeneratorAction::initEventRandomSeed(G4Event* anEvent) {

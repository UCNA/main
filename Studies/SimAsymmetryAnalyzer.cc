...
std::vector<TH1*> SimAsymmetryAnalyzer::calculateCorrections(AsymmetryAnalyzer& Adat, AsymmetryAnalyzer& Asim) {
	...
	if(false) {
		// cheat: forces MC recon asymmetry to match "real value" ... undoes Data/MC backscatter fracs difference
		TH1* hFinal = (TH1*)asymStages.back()->Clone();
		for(int b=1; b<=hFinal->GetNbinsX(); b++) {
			double e0 = hFinal->GetBinCenter(b);
			//hFinal->SetBinContent(b,correctedAsymmetry(e0));
		}
		asymStages.push_back(hFinal);
	} else {
		// real way: uses asymmetry acceptance to correct each event category
		for(EventType t=TYPE_0_EVENT; t<=TYPE_III_EVENT; ++t) {
			for(Side s = EAST; s <= WEST; ++s) {
				for(AFPState a = AFP_OFF; a<=AFP_ON; ++a) {
					TH1* hDat = Adat.qEnergySpectra[s][nBetaTubes][t]->fgbg[a]->h[GV_OPEN];
					TProfile* pAsAc = (TProfile*)qAsymAcc[s][t]->fgbg[a]->h[GV_OPEN];
					TProfile* pBctAc = (TProfile*)qBCT[s][t]->fgbg[a]->h[GV_OPEN];
					for(int b=1; b<=hDat->GetNbinsX(); b++) {
						...
						//hDat->SetBinContent(b, hDat->GetBinContent(b) *
						//					(1-A0_PDG*(a==AFP_ON?-1:1)*(s==EAST?-1:1)*beta(e0)/2.) /
						//					(1-A0_PDG*(a==AFP_ON?-1:1)*pBctAc->GetBinContent(b)));
					}
				}
			}
		}

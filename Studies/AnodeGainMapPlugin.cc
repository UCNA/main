...
AnodeGainMapPlugin::AnodeGainMapPlugin(RunAccumulator* RA, unsigned int nr): PositionBinnedPlugin(RA,"AnodePos",nr) {
	TH1F hTemplate("hAnode","Anode Signal",175,0,3500);
	for(Side s = EAST; s <= WEST; ++s)
		sectAnode[s] = allocateSegmentHistograms(hTemplate,AFP_OTHER,s);
}

void AnodeGainMapPlugin::fillCoreHists(ProcessedDataScanner& PDS, double weight) {
	Side s = PDS.fSide;
...
void AnodeGainMapPlugin::genAnodePosmap() {

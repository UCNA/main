...
AnodeGainPlugin::AnodeGainPlugin(RunAccumulator* RA): AnalyzerPlugin(RA,"wirechambergain") {
...
AnodeGainPlugin::~AnodeGainPlugin() {
	for(Side s = EAST; s <= WEST; ++s) {
		for(EventType tp = TYPE_0_EVENT; tp <= TYPE_III_EVENT; ++tp) {
			if(gAnode[s][tp]) {
				delete gAnode[s][tp];
...
void AnodeGainPlugin::fillCoreHists(ProcessedDataScanner& PDS, double weight) {
...
void AnodeGainPlugin::calculateResults() {
...
void AnodeGainPlugin::makePlots() {
	for(Side s = EAST; s <= WEST; ++s) {
		for(EventType tp = TYPE_0_EVENT; tp <= TYPE_III_EVENT; ++tp) {
...
void AnodeGainPlugin::compareMCtoData(AnalyzerPlugin* AP) {
	// re-cast to correct type
	AnodeGainPlugin& dat = *(AnodeGainPlugin*)AP;
...
WirechamberChargeProxyPlugin::WirechamberChargeProxyPlugin(RunAccumulator* RA): AnalyzerPlugin(RA,"wirechamberChargeProxy") {
...
void WirechamberChargeProxyPlugin::fillCoreHists(ProcessedDataScanner& PDS, double weight) {
	const Side s = PDS.fSide;
